...
class OutputFilter:
    _PATTERNS: tuple[tuple[str, re.Pattern[str]], ...] = (
        (
            "snakemake_command",
            re.compile(_COMMAND_PREFIX + r"snakemake\s+", re.IGNORECASE),
        ),
        (
            "rm_command",
            re.compile(
                _COMMAND_PREFIX + r"rm\s+(?:-[a-zA-Z]*f|--force)", re.IGNORECASE
            ),
        ),
        (
            "python_command",
            re.compile(
                _COMMAND_PREFIX + r"python3?(?:\.\d+)?\s+\S+\.py", re.IGNORECASE
            ),
        ),
        ...
        (
            "execution_directive",
            re.compile(
                r"\b(?:run|execute)\s+(?:this\s+)?(?:shell\s+)?command\b", re.IGNORECASE
            ),
        ),
    ...
    def filter_text(self, text: str) -> FilterResult:
        matched = [name for name, pattern in self._PATTERNS if pattern.search(text)]
        if matched:
            return FilterResult(
                text=_SAFE_MESSAGE,
                filtered=True,
