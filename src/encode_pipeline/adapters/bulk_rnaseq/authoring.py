...
# MultiQC 1.33 applies these filename-cleaning literals (its defaults plus the
# pinned nf-core/rnaseq 3.26.0 additions) to module sample identities. Every
# literal below can occur in the adapter's identifier alphabet. Reserving them
# keeps the mapping from authored sample ID to machine-report ID lossless.
...
_MULTIQC_SAMPLE_CLEAN_PATTERN = (
    "(?:" + "|".join(re.escape(token) for token in MULTIQC_SAMPLE_CLEAN_TOKENS) + ")"
)
...
def _sample_schema() -> dict[str, object]:
    ...
    sample_identifier = {
        **deepcopy(identifier),
        "not": {"pattern": _MULTIQC_SAMPLE_CLEAN_PATTERN},
    }
    fastq = {
        "type": "string",
        "maxLength": MAX_SAMPLE_CELL_LENGTH,
