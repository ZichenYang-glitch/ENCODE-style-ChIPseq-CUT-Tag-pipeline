...
SNAPSHOTS_DIR = REPO_ROOT / "test" / "fixtures" / "dag_snapshots"
...
def _prepare_profile_workdir(profile):
...
def _run_snakemake_dryrun(config_path):
...
def _snapshot(profile):
    workdir, config_path = _prepare_profile_workdir(profile)
    try:
        return _run_snakemake_dryrun(config_path)
    finally:
...
def _cmd_diff(args):
    rules = _snapshot(args.profile)
    snapshot_path = SNAPSHOTS_DIR / f"{args.profile}.txt"
