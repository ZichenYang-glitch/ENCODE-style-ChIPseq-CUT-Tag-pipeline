...
def _validate_mnase_config(mnase: dict) -> dict:
    ...
    def _resolve_fragment_range(class_name, legacy_mono, hard_default):
        ...
        if class_name in fragments_raw:
            ...
            _validate_range_pair(r, f"mnase.fragments.{class_name}")
...
def _validate_range_pair(r, label):
    ...
    *r* may be a list or tuple. Must have exactly 2 positive ints,
    ...
    lo, hi = (_parse_range_int(r[0], label, r),
              _parse_range_int(r[1], label, r))
    if lo <= 0 or hi <= 0:
        raise ValidationError(
...
def _parse_range_int(value, label, original):
