...
This document defines the workflow's output types, paths, generating methods,
and status. It is the maintained reference for downstream consumers, including
manifest generation, QC reporting, and user documentation. The
[README](../README.md) provides the product-level entry point.

The manifest field schema is generated by `scripts/make_manifest.py` at
`results/multiqc/result_manifest.tsv` with 10 columns:
...
| `macs3_fe_bw` | bedGraphToBigWig | signal_track_fe_bw | `results/<sample>/03_signal/<sample>.FE.bw` | implemented |
| `macs3_ppois_bw` | bedGraphToBigWig | signal_track_ppois_bw | `results/<sample>/03_signal/<sample>.ppois.bw` | implemented |
...
| `qc_summary` | assemble_qc_summary | qc_summary | `results/<sample>/01_qc/<sample>.qc_summary.tsv` | implemented |
...
### MNase-seq single-sample outputs

| output_type | method | rule | path | status |
| :--- | :--- | :--- | :--- | :--- |
| `mnase_sub_bam` | alignmentSieve | mnase_split_sub | `results/<sample>/03_fragments/<sample>.sub.bam` | stable |
| `mnase_sub_bai` | samtools index | mnase_split_sub | `results/<sample>/03_fragments/<sample>.sub.bam.bai` | stable |
...
| `mnase_di_bam` | alignmentSieve | mnase_split_di | `results/<sample>/03_fragments/<sample>.di.bam` | stable |
| `mnase_di_bai` | samtools index | mnase_split_di | `results/<sample>/03_fragments/<sample>.di.bam.bai` | stable |
...
| `mnase_qc_summary` | mnase_qc_summary.py | mnase_qc_summary | `results/<sample>/01_qc/<sample>.mnase_qc_summary.tsv` | stable |

### MNase-seq experiment-level (pooled) outputs
...
| `pooled_fe_bw` | bedGraphToBigWig | pooled_signal_track_fe_bw | `results/experiments/<exp>/03_signal/<exp>.pooled.FE.bw` | implemented |
...
| `qc_flag` | string | Always `NA`; automatic QC threshold conclusions are not implemented. |
