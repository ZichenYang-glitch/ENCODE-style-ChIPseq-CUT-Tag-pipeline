...
SOURCE_OWNED_EXECUTION_CONTRACTS = (
    ...
    "default-execution-qualification-1.0.0.json",
)
...
def test_wheel_ships_exact_pinned_nfcore_contracts(tmp_path: Path) -> None:
    ...
    with zipfile.ZipFile(wheels[0]) as archive:
        ...
        qualification = json.loads(
            source_owned_contracts["default-execution-qualification-1.0.0.json"]
        )
        persistence_contract = json.loads(
            source_owned_contracts["execution-persistence-contract-1.0.0.json"]
        )
        record_names = [
