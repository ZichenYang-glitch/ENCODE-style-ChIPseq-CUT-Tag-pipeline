#!/usr/bin/env python3
"""Protect TSS and ATAC validation, DAG, and command behavior."""
...
from test_optional_qc_dag import (  # noqa: E402
    _DISABLED_QC,
    _make_config,
...
def test_tss_true_missing_gtf_fails_validation():
    td = tempfile.mkdtemp(prefix="tss_validate_")
    try:
        row = _sample(td)
...
def test_tss_true_schedules_tss_rules():
    td = tempfile.mkdtemp(prefix="tss_dag_")
    try:
        row = _sample(td)
        _touch_inputs(row)
...
def test_atac_narrow_validates():
    td = tempfile.mkdtemp(prefix="atac_validate_")
    try:
...
def test_atac_dryrun_uses_shared_pipeline_without_cuttag_sidecars():
    td = tempfile.mkdtemp(prefix="atac_dag_")
    try:
        row = _sample(td, assay="atac", peak_mode="narrow")
        _touch_inputs(row)
        samples_tsv = os.path.join(td, "samples.tsv")
