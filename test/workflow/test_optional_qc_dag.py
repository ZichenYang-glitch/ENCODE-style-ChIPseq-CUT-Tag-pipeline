#!/usr/bin/env python3
"""Validate optional QC rule gates and MultiQC wiring.
...
Uses real validate_samples.py calls and isolated Snakemake dry-runs.
"""
...
def _run_snakemake_dryrun(config_path, samples_path, extra_args=None, quiet=True):
    ...
    cmd = [
        SNAKEMAKE,
        "-s",
        os.path.join(_REPO, "workflow", "Snakefile"),
        "--configfile",
        config_path,
        "--dry-run",
        "--use-conda",
...
def test_missing_qc_block_defaults_new_keys_false():
    ...
    td = tempfile.mkdtemp(prefix="optional_qc_defaults_")
    try:
...
def test_all_new_keys_true_validates():
    ...
    td = tempfile.mkdtemp(prefix="optional_qc_enabled_")
    try:
...
def test_all_new_keys_false_validates():
    ...
    td = tempfile.mkdtemp(prefix="optional_qc_disabled_")
    try:
        samples_tsv = os.path.join(td, "samples.tsv")
        _write_samples_tsv(samples_tsv, [_chipseq_sample()])
        config_yml = os.path.join(td, "config.yaml")
...
def test_invalid_boolean_cross_correlation_rejected():
    ...
    td = tempfile.mkdtemp(prefix="optional_qc_invalid_")
    try:
        samples_tsv = os.path.join(td, "samples.tsv")
        _write_samples_tsv(samples_tsv, [_chipseq_sample()])
        config_yml = os.path.join(td, "config.yaml")
...
def test_picard_metrics_true_missing_ref_fails():
    ...
    td = tempfile.mkdtemp(prefix="optional_qc_reference_")
    try:
        samples_tsv = os.path.join(td, "samples.tsv")
        _write_samples_tsv(samples_tsv, [_chipseq_sample()])
        config_yml = os.path.join(td, "config.yaml")
...
def _create_dryrun_env():
    ...
    """
    td = tempfile.mkdtemp(prefix="optional_qc_dag_")
    samples_tsv = os.path.join(td, "samples.tsv")
    s1 = _chipseq_sample("s1")
    s1["fastq_1"] = os.path.join(td, "s1_R1.fq.gz")
    s1["bowtie2_index"] = os.path.join(td, "index", "hg38")
...
def test_cross_correlation_true_in_dag():
    ...
    try:
        ...
        assert "cross_correlation" in output, (
            "Expected cross_correlation rule in dry-run jobs list"
        )
    finally:
        shutil.rmtree(td, ignore_errors=True)
...
def test_preseq_complexity_true_in_dag():
    ...
    try:
        ...
        assert "preseq_complexity" in output, (
            "Expected preseq_complexity rule in dry-run jobs list"
        )
    finally:
        shutil.rmtree(td, ignore_errors=True)
...
def test_picard_metrics_true_in_dag():
    ...
    try:
        ...
        assert "picard_collect_multiple_metrics" in output, (
            "Expected picard_collect_multiple_metrics rule in dry-run jobs list"
        )
    finally:
        shutil.rmtree(td, ignore_errors=True)
...
def test_all_disabled_dag_unchanged():
    """Disabled optional QC modules do not enter the DAG."""
    td = _create_dryrun_env()
    try:
        samples_tsv = os.path.join(td, "samples.tsv")
        config_yml = os.path.join(td, "config.yaml")
        ...
        assert "cross_correlation" not in output, (
            "Should not see cross_correlation in DAG when disabled"
        )
        assert "preseq_complexity" not in output, (
            "Should not see preseq_complexity in DAG when disabled"
        )
        assert "picard_collect_multiple_metrics" not in output, (
            "Should not see picard_collect_multiple_metrics in DAG when disabled"
        )
    finally:
        shutil.rmtree(td, ignore_errors=True)
...
def test_explicit_cross_correlation_target_resolves():
    ...
    try:
        ...
        assert rc == 0, f"Explicit target failed: rc={rc}\nstderr: {stderr}"
    finally:
        shutil.rmtree(td, ignore_errors=True)


# --- Cross-correlation summary DAG tests ---------------------------------
...
def test_cross_corr_summary_explicit_target():
    ...
    try:
        ...
        target = os.path.join(td, "results", "multiqc", "cross_correlation_summary.tsv")
        rc, stdout, stderr = _run_snakemake_dryrun(
            config_yml,
            samples_tsv,
            extra_args=[target],
...
def test_multiqc_report_target_with_cross_corr_resolves():
    ...
    try:
        ...
        assert rc == 0, f"MultiQC report target failed: rc={rc}\nstderr: {stderr}"
        output = stdout + stderr
        assert "cross_correlation_summary" in output
